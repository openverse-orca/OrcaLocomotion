...
# The XRoboToolkit binding links against the active Conda environment's C++
# gRPC runtime when built with scripts/install_xrobottoolkit_sdk.sh.
if [[ -n "${CONDA_PREFIX:-}" && -d "${CONDA_PREFIX}/lib" ]]; then
  export LD_LIBRARY_PATH="${CONDA_PREFIX}/lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
fi

exec "${PYTHON_BIN}" -m orca_rl.play_g1_heft_pico \
